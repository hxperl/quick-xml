    ...
    E: EntityResolver,
{
    ...
    /// [`TEXT_KEY`].
    has_text_field: bool,
    /// `true` after the [`DeEvent::End`] event that closes [`Self::start`] was
    /// consumed by [`next_key_seed()`]. Used by [`Self::end()`] to check if the
    /// visitor has driven this accessor to completion.
    ///
    /// [`next_key_seed()`]: MapAccess::next_key_seed
    finished: bool,
}
    ...
    E: EntityResolver,
{
    ...
    /// Finishes reading of the element.
    ///
    /// A [`Visitor`] is not obliged to read a [`MapAccess`] until it returns
    /// `None`, and when it does not, the [`DeEvent::End`] event that closes
    /// [`Self::start`] -- and possibly some content before it -- is still
    /// pending. Leaving those events in the stream would desynchronize the
    /// parent deserializer, so skip them here.
    ///
    /// This is a no-op when the visitor did read the accessor to its end, which
    /// is what the `Visitor`s generated by `#[derive(Deserialize)]` for structs
    /// do.
    fn end(self) -> Result<(), DeError> {
        if self.finished {
            return Ok(());
        }
        // `read_to_end()` should be called after `next()` returned the `Start`
        // event of `self.start`, which is the contract of `new()`
...
/// Deserializes a struct or a map from the element started by the
/// [`DeEvent::Start`] event `start`.
///
/// Unlike a bare `visitor.visit_map(ElementMapAccess::new(...))` call, this
/// skips the rest of the element if the visitor did not read the accessor to
/// its end. See [`ElementMapAccess::end()`] for why that is necessary.
pub(crate) fn deserialize_element<'de, 'd, R, E, V>(
    de: &'d mut Deserializer<'de, R, E>,
    start: BytesStart<'de>,
    fields: &'static [&'static str],
    visitor: V,
...
where
    ...
    V: Visitor<'de>,
{
    let mut map = ElementMapAccess::new(de, start, fields)?;
    ...
    E: EntityResolver,
{
    ...
    {
        match self.map.de.next()? {
            DeEvent::Start(e) => deserialize_element(self.map.de, e, fields, visitor),
            DeEvent::Text(e) => {
    ...
    E: EntityResolver,
{
    ...
    where
        V: Visitor<'de>,
    {
        deserialize_element(self.de, self.start, fields, visitor)
    }
